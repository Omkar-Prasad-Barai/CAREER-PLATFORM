...
import { BookOpen, Users, User, FileText, Pencil, GraduationCap, Briefcase, Globe, Linkedin, LayoutDashboard, Lightbulb, Loader2 } from 'lucide-react';
...
const ProfessorDashboard = () => {
    ...
    const [activeView, setActiveView] = useState('post-project');
    ...
    useEffect(() => {
        if (user) {
            if (Array.isArray((user as any).mentoringInterests)) setMentoringInterests((user as any).mentoringInterests);
            setInstitute((user as any).institute || '');
            setDepartment((user as any).department || '');
            setDomainExpertise((user as any).domainExpertise || '');
            setBio((user as any).bio || '');
            setTagline((user as any).tagline || '');
        }
    }, [user]);

    const addMentoringInterest = () => {
        const trimmed = newSkill.trim();
        if (trimmed && !mentoringInterests.includes(trimmed)) {
            setMentoringInterests(prev => [...prev, trimmed]);
        }
        setNewSkill('');
    };
    ...
    const handleSaveProfile = async () => {
        ...
        try {
            ...
            const profileData: Record<string, unknown> = { tagline, bio, mentoringInterests, institute, department, domainExpertise };
    ...
    const handleStatusChange = (id: number, newStatus: string) => {
        setApplicationsData(prev => prev.map(app => app.id === id ? { ...app, status: newStatus } : app));
    };

    const sidebarItems: SidebarItem[] = [
        { icon: BookOpen, label: 'Post Project', href: '#', onClick: () => setActiveView('post-project') },
        { icon: BookOpen, label: 'Manage Our Posts', href: '#', onClick: () => setActiveView('manage-posts') },
        { icon: Users, label: 'View Applications', href: '#', onClick: () => setActiveView('applications') },
        { icon: User, label: 'Our Professor Profile', href: '#', onClick: () => setActiveView('profile') },
    ];

    const VIEW_TO_LABEL: Record<string, string> = {
        'post-project': 'Post Project', 'manage-posts': 'Manage Our Posts',
        applications: 'View Applications', profile: 'Our Professor Profile', settings: 'Account Settings',
    };
    const activeViewLabel = VIEW_TO_LABEL[activeView] || 'Post Project';

    return (
        <DashboardLayout sidebarItems={sidebarItems} role="professor" onSettingsClick={() => setActiveView('settings')} activeViewLabel={activeViewLabel} onOverviewClick={() => setActiveView('post-project')}>

            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                {activeView === 'post-project' && <PostProjectForm />}
                ...
                {activeView === 'applications' && (
                     <SharedApplicationsList
                        ...
                        columns={[
                            ...
                            { header: "Project", accessor: "project", className: "text-gray-600" },
                            { header: "GPA", accessor: "gpa", className: "text-gray-500" },
                            {
                                header: "Status",
                                accessor: (item) => (
                                    <span className={clsx(
                    ...
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        ...
                        <div className="backdrop-blur-md border border-slate-200/80 rounded-2xl p-6 bg-gradient-to-br from-white via-slate-50 to-indigo-50/40 shadow-glow-indigo">
                            <div className="flex flex-col md:flex-row items-center md:items-start gap-6">
                                <InitialsAvatar fullName={user?.fullName || 'Professor'} size="lg" />
                                <div className="flex-1 text-center md:text-left">
                                    <h2 className="text-2xl font-bold text-slate-800 mb-1">{user?.fullName || 'Professor'}</h2>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            ...
                            <div className="bg-gradient-to-br from-white to-sky-50/60 border border-slate-200 rounded-xl p-5 shadow-glow-sky card-premium relative group">
                                <button
                                    onClick={() => setEditingField(editingField === 'academic' ? null : 'academic')}
                                ...
                                <div className="flex items-center gap-2 mb-3">
                                    ...
                                    <h3 className="font-semibold text-slate-800">Academic Details</h3>
                            </div>
                            {editingField === 'academic' ? (
                                <div className="space-y-2">
                                    <input type="text" value={institute} onChange={e => setInstitute(e.target.value)} placeholder="Institute Name" className="w-full px-3 py-1.5 bg-white border border-slate-300 rounded-lg text-sm text-slate-800 placeholder-slate-400 outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 transition-all" />
                                    <input type="text" value={department} onChange={e => setDepartment(e.target.value)} placeholder="Department" className="w-full px-3 py-1.5 bg-white border border-slate-300 rounded-lg text-sm text-slate-800 placeholder-slate-400 outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 transition-all" />
                                </div>
                            ) : (
                                <>
                                    <p className="text-slate-600 text-sm">{institute || 'Institute not specified'}</p>
                                    <p className="text-slate-500 text-xs mt-1">{department || 'Department not specified'}</p>
                                </>
                            )}
                            </div>
                            ...
                            <div className="bg-gradient-to-br from-white to-indigo-50/60 border border-slate-200 rounded-xl p-5 shadow-glow-sky card-premium relative group">
                                <button
                                    onClick={() => setEditingField(editingField === 'domain' ? null : 'domain')}
                                    className="absolute top-4 right-4 text-slate-400 hover:text-indigo-600 transition-colors duration-150 opacity-0 group-hover:opacity-100"
                                >
                                    <Pencil className="w-4 h-4" />
                                </button>
                                <div className="flex items-center gap-2 mb-3">
                                    ...
                                    <h3 className="font-semibold text-slate-800">Domain Expertise</h3>
                            </div>
                            {editingField === 'domain' ? (
                                <input type="text" value={domainExpertise} onChange={e => setDomainExpertise(e.target.value)} placeholder="Your domain expertise" className="w-full px-3 py-1.5 bg-white border border-slate-300 rounded-lg text-sm text-slate-800 placeholder-slate-400 outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 transition-all" />
                            ) : (
                                <p className="text-slate-600 text-sm">{domainExpertise || 'No domain expertise specified'}</p>
                            )}
                            </div>

                            {/* Card 3 — Mentoring Interests */}
                            <div className="bg-gradient-to-br from-white to-violet-50/60 border border-slate-200 rounded-xl p-5 shadow-glow-sky card-premium relative group">
                                <button
                                    onClick={() => setEditingField(editingField === 'mentoring' ? null : 'mentoring')}
                                    className="absolute top-4 right-4 text-slate-400 hover:text-indigo-600 transition-colors duration-150 opacity-0 group-hover:opacity-100"
                                >
                                    <Pencil className="w-4 h-4" />
                                </button>
                                <div className="flex items-center gap-2 mb-3">
                                    ...
                                    <h3 className="font-semibold text-slate-800">Mentoring Interests</h3>
                                </div>
                                <div className="flex flex-wrap gap-2">
                                    {mentoringInterests.map((interest, i) => (
                                        <span key={i} className="bg-indigo-50 text-indigo-700 border border-indigo-200 px-3 py-1 rounded-full text-xs shadow-sm shadow-indigo-100/50">
                                            {interest}
                                            {editingField === 'mentoring' && (
                                                <button
                                                    onClick={() => setMentoringInterests(prev => prev.filter((_, idx) => idx !== i))}
                                                    className="ml-1.5 text-indigo-400 hover:text-indigo-700"
                                                >×</button>
                                            )}
                                        </span>
                                    ))}
                                    {mentoringInterests.length === 0 && (
                                        <span className="text-slate-500 text-xs">No mentoring interests added yet</span>
                                    )}
                                </div>
                                {editingField === 'mentoring' && (
                                    <div className="mt-3 flex items-center gap-2">
                                        <input
                                            ...
                                            onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); addMentoringInterest(); } }}
                                ...
                                {editingField === 'about' ? (
                                    <div>
                                        <textarea
                                            ...
                                            placeholder="Describe your research interests, teaching philosophy..."
                                            rows={3}
                        ...
                        <div className="border border-slate-200 rounded-2xl p-6 bg-gradient-to-br from-slate-50 to-indigo-50/30 shadow-sm">
                            <div className="flex items-center gap-2 mb-4">
                                ...
                                <h3 className="font-semibold text-slate-800">Research Papers / CV</h3>
                            </div>
                            <ResumeDragDrop
                        ...
                        <div className="border border-slate-200 rounded-2xl p-6 bg-gradient-to-br from-white to-slate-50 shadow-sm">
                            ...
                            <div className="flex flex-wrap gap-6">
                                ...
                                <a href="#" className="flex items-center gap-2 text-emerald-600 hover:text-emerald-700 hover:underline font-medium transition-colors duration-200">
                                    <Globe className="w-5 h-5" /> Research Gate / Personal Website
                                </a>
                            </div>
                        </div>
