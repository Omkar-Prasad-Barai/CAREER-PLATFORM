...
import { Users, User, Search, FileText, Pencil, Building2, Globe, Linkedin, Briefcase, Loader2 } from 'lucide-react';
...
const RecruiterDashboard = () => {
    ...
    const [activeView, setActiveView] = useState('find-candidates');
    ...
    useEffect(() => {
        if (user) {
            ...
            setWorkEmail((user as any).workEmail || '');
    ...
    const handleSaveProfile = async () => {
        ...
        try {
            ...
            const profileData: Record<string, unknown> = { tagline, bio, companyName, workEmail };
    ...
    const handleStatusChange = (id: number, newStatus: string) => {
        setCandidatesData(prev => prev.map(app => app.id === id ? { ...app, status: newStatus } : app));
    };

    const sidebarItems: SidebarItem[] = [
        { icon: Search, label: 'Find Candidates', href: '#', onClick: () => setActiveView('find-candidates') },
        { icon: Search, label: 'Manage Our Posts', href: '#', onClick: () => setActiveView('manage-posts') },
        { icon: Users, label: 'My Candidates', href: '#', onClick: () => setActiveView('candidates') },
        { icon: Users, label: 'Browse Talent', href: '#', onClick: () => setActiveView('browse-talent') },
        { icon: User, label: 'Our Recruiter Profile', href: '#', onClick: () => setActiveView('profile') },
    ];

    const VIEW_TO_LABEL: Record<string, string> = {
        'find-candidates': 'Find Candidates', 'manage-posts': 'Manage Our Posts',
        candidates: 'My Candidates', 'browse-talent': 'Browse Talent', profile: 'Our Recruiter Profile', settings: 'Account Settings',
    };
    const activeViewLabel = VIEW_TO_LABEL[activeView] || 'Find Candidates';

    return (
        <DashboardLayout sidebarItems={sidebarItems} role="recruiter" onSettingsClick={() => setActiveView('settings')} activeViewLabel={activeViewLabel} onOverviewClick={() => setActiveView('find-candidates')}>

            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                {activeView === 'find-candidates' && <PostJobForm />}
                ...
                {activeView === 'candidates' && (
                    <SharedApplicationsList 
                        ...
                        data={candidatesData}
                        columns={[
                            { header: "Candidate Name", accessor: "name", className: "font-semibold text-gray-900" },
                            { header: "Target Role", accessor: "role", className: "text-gray-600" },
                            { header: "Client", accessor: "client", className: "text-gray-500" },
                            { 
                                header: "Status", 
                                accessor: (item) => (
                                    <span className={clsx(
                    ...
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        ...
                        <div className="backdrop-blur-md border border-slate-200/80 rounded-2xl p-6 bg-gradient-to-br from-white via-slate-50 to-indigo-50/40 shadow-glow-indigo">
                            <div className="flex flex-col md:flex-row items-center md:items-start gap-6">
                                <InitialsAvatar fullName={user?.fullName || 'Recruiter'} size="lg" />
                                <div className="flex-1 text-center md:text-left">
                                    <h2 className="text-2xl font-bold text-slate-800 mb-1">{user?.fullName || 'Recruiter'}</h2>
                                    ...
                                    <input
                                        ...
                                        placeholder="Write a short tagline about your recruiting expertise..."
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            ...
                            <div className="bg-gradient-to-br from-white to-sky-50/60 border border-slate-200 rounded-xl p-5 shadow-glow-sky card-premium relative group">
                                ...
                                <p className="text-slate-600 text-sm">{(user as any)?.companyName || 'Company not specified'}</p>
                                <p className="text-slate-500 text-xs mt-1">{user?.email || 'Work email not specified'}</p>
                            </div>

                            {/* Card 2 — Recruitment Focus */}
                            <div className="bg-gradient-to-br from-white to-indigo-50/60 border border-slate-200 rounded-xl p-5 shadow-glow-sky card-premium relative group">
                                ...
                                <div className="flex items-center gap-2 mb-3">
                                    ...
                                    <h3 className="font-semibold text-slate-800">Recruitment Focus</h3>
                                </div>
                                <p className="text-slate-500 text-sm">Specializing in sourcing top talent for organizations across multiple industries.</p>
                            </div>
                            ...
                            <div className="bg-gradient-to-br from-white to-violet-50/60 border border-slate-200 rounded-xl p-5 shadow-glow-sky card-premium relative group">
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <div>
                                        ...
                                        <p className="text-slate-400 text-xs">Hires This Year</p>
                                    </div>
                                    <div>
                                        <p className="text-2xl font-bold text-indigo-600">0</p>
                                        <p className="text-slate-400 text-xs">Active Roles</p>
                                    </div>
                                </div>
                            </div>
                                ...
                                {editingField === 'about' ? (
                                    <div>
                                        <textarea
                                            ...
                                            placeholder="Describe your recruiting experience and specializations..."
                                            rows={3}
