...
const StudentDashboard = () => {
    ...
    // Editable field state initialized from user data
    const [collegeName, setCollegeName] = useState('');
    const [degree, setDegree] = useState('');
    const [graduationYear, setGraduationYear] = useState('');
    const [experience, setExperience] = useState('');

    // Initialize profile skills from user data
    useEffect(() => {
        if (user) {
    ...
    // Build "My Applications" from applied IDs
    ...
    // Filter opportunities by tab + drawer filters
    const filterByTab = (tab: string) => {
        ...
        if (tab !== 'all') {
            filtered = filtered.filter(opp => {
                ...
                if (tab === 'job') return jt.includes('full') || jt.includes('contract') || cat.includes('job') || cat.includes('engineer') || (!jt.includes('intern') && !jt.includes('part'));
    ...
    const handleSaveProfile = async () => {
        ...
        try {
            ...
            if (resumeFile) {
                ...
                resumeData = { resume: res.resumeUrl };
                setResumeFile(null);
            }
            // Save text fields
    ...
    return (
        <DashboardLayout sidebarItems={sidebarItems} role="student" onSettingsClick={() => setActiveView('settings')} activeViewLabel={activeViewLabel} onOverviewClick={() => setActiveView('overview')}>
            ...
            {activeView === 'dashboard' && (
                <>
                    ...
                    {/* Opportunities Grid (full width — no sidebar) */}
                ...
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    <div className="backdrop-blur-md border border-slate-200/80 rounded-2xl p-6 bg-gradient-to-br from-white via-slate-50 to-indigo-50/40 shadow-glow-indigo">
                        <div className="flex flex-col md:flex-row items-center md:items-start gap-6">
                            <InitialsAvatar fullName={user?.fullName || 'Student'} size="lg" />
                            <div className="flex-1 text-center md:text-left">
                                <h2 className="text-2xl font-bold text-slate-800 mb-1">{user?.fullName || 'Student'}</h2>
